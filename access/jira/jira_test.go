...
func (s *JiraSuite) SetUpTest(c *C) {
	...
	s.publicURL = ""
	s.fakeJira = NewFakeJIRA(jira.User{Name: "Test User", EmailAddress: s.me.Username + "@example.com"}, RaceNumber)
}
...
func (s *JiraSuite) TestRace(c *C) {
		...
		process.SpawnCritical(func(ctx context.Context) error {
			...
			for {
				...
				if status := resp.StatusCode; status != http.StatusOK {
					...
					runtime.Gosched() // time.Sleep is not good here since we're simulating a tight race.
				} else {
					return nil
				}
			}
	...
	c.Assert(count, Equals, RaceNumber)
}
